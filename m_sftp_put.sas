...
%macro m_sftp_put(get_directory=, put_directory=, put_file1=, put_file2=, put_cleanup=NONE);
	...
	%if &put_cleanup. ne NONE %then %do;
		...
		%if &c_s_sftp_put_production. = Y %then %do;
			...
			/* SASDOC --------------------------------------------------------------------------------------
			|  STEP - clean up PROD SFTP server - EA PROD
			+---------------------------------------------------------------------------------------------*/		
			data _null_;
			file "&put_linux_directory./put.txt";
			put "cd &put_etl_directory.";
			put "rm &put_etl_directory./CPL*"; 
			put "rm &put_etl_directory./SFMC*";
			put "exit";
			run; 
		...
		%else %if &c_s_sftp_put_production. = N %then %do;
			...
			/* SASDOC --------------------------------------------------------------------------------------
			|  STEP - clean up IT TEST and QA SFTP server - EA PREPROD
			+---------------------------------------------------------------------------------------------*/		
			data _null_;
			file "&put_linux_directory./put.txt";
			put "cd &put_etl_directory.";
			...
			data _null_;
			rc = system("cd &put_linux_directory."); 
			rc = system("sftp &sftp_eoms_id.@tstwebtransport.caremark.com < &put_linux_directory./put.txt"); 
			if   _error_=0  then do;
			call symput('file_sftp_flg','1');
			end;
			else  call symput('file_sftp','0');
	...
	%else %do;
		...
		%if &c_s_sftp_put_production. = Y %then %do;
			...
			%do g = 1 %to &total_sftp;
				...
				options="-oKexAlgorithms=diffie-hellman-group14-sha1" host="&sftp_eoms_id.@webtransport.caremark.com" wait_milliseconds=4000 debug;
		...
		%else %if &c_s_sftp_put_production. = N %then %do;
			...
			|  STEP - Create listing of files to SFTP
			+---------------------------------------------------------------------------------------------*/	
			%put NOTE: Get cpl and sfmc files from the test SFTP server - eaz1etlada1a ; 
			...
			/* SASDOC --------------------------------------------------------------------------------------
			|  STEP - IT DEV + QA + EA PREPROD SFTP server
			+---------------------------------------------------------------------------------------------*/		

			%do g = 1 %to &total_sftp;

				filename getfile "/anr_dep/communication_history/eoms_IT/&&sftpname&g" recfm=f;
				filename putfile sftp "/appl/edw/incoming/campgn/&&sftpname&g" recfm=f 
				options="-oKexAlgorithms=diffie-hellman-group14-sha1" host="ftpuser@eaz1etlada1a" wait_milliseconds=4000 debug;
				...
				filename getfile "/anr_dep/communication_history/eoms_IT/&&sftpname&g" recfm=f;
				filename putfile sftp "/appl/edw/incoming/campgn/&&sftpname&g" recfm=f 
				options="-oKexAlgorithms=diffie-hellman-group14-sha1" host="ftpuser@eaz1etlaua1a" wait_milliseconds=4000 debug;
				...
				filename getfile "/anr_dep/communication_history/eoms_IT/&&sftpname&g" recfm=f;
				filename putfile sftp "/ant591/IVR/incoming/&&sftpname&g" recfm=f 
				options="-oKexAlgorithms=diffie-hellman-group14-sha1" host="&sftp_eoms_id.@tstwebtransport.caremark.com" wait_milliseconds=4000 debug;
			...
			/* SASDOC --------------------------------------------------------------------------------------
			|  STEP - chmod for files on IT TEST and QA SFTP server - DEV
			+---------------------------------------------------------------------------------------------*/
			data _null_;
			file "&put_linux_directory./put.txt";
			put "cd /appl/edw/incoming/campgn";
			put "chmod 777 *"; 
			put "exit";
			run; 
			...
			data _null_;
			rc = system("cd &put_linux_directory."); 
			rc = system("sftp ftpuser@eaz1etlada1a < &put_linux_directory./put.txt"); 
			if   _error_=0  then do;
			call symput('file_sftp_flg','1');
			end;
			else  call symput('file_sftp','0');
			...
			/* SASDOC --------------------------------------------------------------------------------------
			|  STEP - chmod for files on IT TEST and QA SFTP server - SIT
			+---------------------------------------------------------------------------------------------*/		
			data _null_;
			file "&put_linux_directory./put.txt";
			put "cd /appl/edw/incoming/campgn";
			...
			data _null_;
			rc = system("cd &put_linux_directory."); 
			rc = system("sftp ftpuser@eaz1etlaua1a < &put_linux_directory./put.txt"); 
			if   _error_=0  then do;
			call symput('file_sftp_flg','1');
			end;
			else  call symput('file_sftp','0');
			...
			/* SASDOC --------------------------------------------------------------------------------------
			|  STEP - chmod for files on IT TEST and QA SFTP server - EA PREPROD
			+---------------------------------------------------------------------------------------------*/
			data _null_;
			file "&put_linux_directory./put.txt";
...
%mend m_sftp_put;
%**m_sftp_put(get_directory=//anr_dep/communication_history/eoms_IT, put_directory=/appl/edw/incoming/campgn, get_file1=test.txt, get_file2=sfmc*);