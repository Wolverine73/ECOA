...
%let c_s_rootdir = %str(/anr_dep/fs01/dqi/prod);		/* root directory for the aprimo request  */
%let c_s_program = %str(bsc);				/* next 1st level down from root directory  */
%let c_s_proj = %str(1746801_1696903_cpy); 				/* next 2md level down from program directory  */
%let c_s_aprimo_activity = 1746801;				/* aprimo activity ID - parent ID for aprimo request (6 digit numeric value )*/
%let c_s_ticket = 1696903;		/* If ONE project in activity, populate with that ONLY project ID; if MEMBER and PRESCRIBER project in activity, populate with MEMBER project ID*/
...
%let c_s_email_to = %str(Hyukjae.yoo@CVSHealth.com);		/* users email address for email alerts */
%let c_s_email_subject = %str(Formulary CFC Program Results);	/* email subject for email alerts */

/*---------------------------------Teradata Database Variables--------------------------*/
%let td_uid=%str(z258057);			/* users ID for teradata */
...
%let c_s_client_nm = %str(BSCA);			/* client name to be used in vendor mailing file */
...
%let c_s_dirfilenm = %str(Legacy Full True Accumulation_APR 2022_UPDATED BSC Custom);				/* name of drug intake form - xlsx */ 

%let c_s_clfilenm = %str(Client Intake Form_July 1 Renewals_041422);	               		/* name of client intake form - xlsx (add values if no client level 1 - c_s_clnt_lvl1)*/

%let c_s_mbfilenm = %str();	     				/* name of member intake form - xlsx */

%let c_s_mbr_i_e=%str();					/* Put 'E' if Exclusion and 'I' if Inclusion*/
...
%let c_s_end_date = %str('2022-05-17');		/* claims end   date for the claims targeting */
%let c_s_elg_end  = %str('2022-05-18');		/* eligibility  date for the the member coverage and the client level 3 */
%let c_s_maildate = %str('2022-07-01');		/* effective    date for the vendor file */
...
%let c_s_prior_cmctn = Y;           /* remove prior communications sent from mailing campaign - member and physician */
%let c_s_cmctn_history_ticket = Y;
%let c_s_cmctn_history_ticket_values = %str('1673238'); /* */
/* SASDOC --------------------------------------------------------------------------------------(dashes)
