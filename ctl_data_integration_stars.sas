...
/* HEADER---------------------------------------------------------------------------------------
| MACRO:    ctl_data_integration_stars
|
| LOCATION:  
| 
| PURPOSE:  
|
| LOGIC:    execute on Tuesday + Thursday + Saturday @ 12:00 pm CST 
|
| INPUT:     
|
| OUTPUT:    
...
%let c_s_client_nm 	= %str(STARS);					/* client name to be used in vendor mailing file */
...
%let c_s_dqi_campaign_id= 86;   						/* dqi campaign id - dss_cea2.dqi_campaigns */
%let c_s_run_validation = Y;							/* DO NOT CHANGE */
%let sashost 		= CAPL1P;
%let execution_date     = 0;
%let ecoa_test		= NO; 							/* NO + YES1B + YES2B + YES3A B + YES4A B + YES5B + YES6A B **/
%let ecoa_zero		= NO;
%let c_s_check_it_disp  = NO;							/* YES = check in dwv_campgn.event_message + NO = testing disp code **/
...
%let c_s_email_subject 	= %str(STARS Data Integration);			/* DO NOT CHANGE - email subject for email alerts */
...
%let c_s_log =data_integration_stars_&c_s_dqi_campaign_id._&c_s_log_date..log;


proc printto log = "&c_s_logdir./&c_s_log." new;
run;	
	
	%include "&c_s_core_dir./cd_data_integration_stars.sas";	
