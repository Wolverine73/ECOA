...
%let c_s_proj 		= %str(eoms_cmctn_history_test);  		/* next 2nd level down from program directory  */
...
%let sftp_eoms_id		= %str(uy2huh5a); 
%let c_s_dqi_production		= N; 		/* prod environment = Y + dev environment = N **/
%let c_s_sftp_get_production	= Y;		/* options servers -  webtransport.caremark.com = Y + tstwebtransport.caremark.com = N + dont send = X **/
%let c_s_sftp_put_production	= X;		/* options servers -  prod = eaz1etlapa1d = Y + dev = eaz1etlada1a= N + dont send = X  **/
%let c_s_files_raw      	= Y;		/* process files in EA yes = Y + process files in EA no = N **/

%let c_s_it_send 		= N;		/* sftp files to prod IT yes = Y(c_s_dqi_production=Y) + sftp files to dev IT yes = Y(c_s_dqi_production=N) + sftp files to IT no = N **/
%let c_s_cmctn_send     	= N;		/* sftp files to prod CMCTN yes = Y (c_s_dqi_production=Y) + sftp files to CMCTN dev = Y (c_s_dqi_production=N) + sftp files CMCTN no = N **/
%let c_s_level1_disposition 	= Y;		/* CMCTN History if letter or email has L1 disposition of 1. = y = prod + n = test cmctn files **/
...
/** logs = /mbr_engmt/reserve/SASData/PROD/data_integration/eoms_cmctn_history_test **/
